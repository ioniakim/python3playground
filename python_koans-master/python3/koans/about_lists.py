...
class AboutLists(Koan):
    ...
    def test_accessing_list_elements(self):
        ...
        self.assertEqual(noms[-4], noms[0])
        self.assertEqual(noms[-1], noms[3])
        self.assertEqual(noms[3], noms[-1])
        self.assertEqual(noms[1], noms[-3])
    ...
    def test_slicing_lists(self):
        ...
        self.assertEqual(list(), noms[4:0])
    ...
    def test_insertions(self):
        ...
        knight.insert(2, 'not')
        temp[2:2] = ['not']
        self.assertEqual(temp, knight)

        knight.insert(0, 'Arthur')
        self.assertEqual(['Arthur'] + temp, knight)

    def test_popping_lists(self):
        stack = [10, 20, 30, 40]
        result = stack[:]
        stack.append('last')
        result[len(result):] = ['last']
